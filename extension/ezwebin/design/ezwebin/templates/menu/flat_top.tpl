<div class="topmenu-design">
    ...
    {if $top_menu_items|gt(0)}
       {foreach $top_menu_items as $item}
            ...
            {else}
      		<li id="node_id_{$item.node_id}" {if and($current_node_id, gt($module_result.path|count, $pagerootdepth), eq( $item.node_id, $module_result.path[$pagerootdepth].node_id ))}class="selected"{/if}><div><a href={$item.url_alias|ezurl}><span>{$item.name|wash()}</span></a></div></li>
    		{/if}
      	{/foreach}
    {/if}
