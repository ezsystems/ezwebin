<div class="topmenu-design">
        ...
        {if $top_menu_items_count}
           {foreach $top_menu_items as $key => $item}
                ...
                {if eq( $item.class_identifier, 'link')}
                    <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><div><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezurl}{else}href={$item.data_map.location.content|ezurl}{if and( is_set( $item.data_map.open_in_new_window.data_int ), $item.data_map.open_in_new_window.data_int )} target="_blank"{/if}{/if}{if $pagedata.is_edit} onclick="return false;"{/if} title="{$item.data_map.location.data_text|wash}"{if $website_toolbar_access} class="menu-item-link" rel={$item.url_alias|ezurl}{/if}><span>{$item.name|wash()}</span></a></div></li>
                {else}
    ...
    {/if}
    {undef $level_2_items $root_node $top_menu_items $item_class $top_menu_items_count $current_node_in_path $current_node_in_path_2 $website_toolbar_access}
