    ...
    {if $top_menu_items_count}
       {foreach $top_menu_items as $key => $item}
            ...
            {if eq( $item.class_identifier, 'link')}
                <li id="node_id_{$item.node_id}"{if $item_class} class="{$item_class|implode(" ")}"{/if}><div><a {if eq( $ui_context, 'browse' )}href={concat("content/browse/", $item.node_id)|ezurl}{else}href={$item.data_map.location.content|ezurl} target="_blank"{/if}{if eq( $ui_context, 'edit' )} onclick="return false;"{/if} title="{$item.data_map.location.data_text|wash}"><span>{$item.name|wash()}</span></a></div></li>
            {else}
